...
func TestBlock_BattlefrontBastionAloneFiresBonus(t *testing.T) {
	...
	got := sim.EvalOneTurnForTesting(d, sim.Matchup{IncomingDamage: 5}, gameengine.Spec{}, hand).Value
	if got != 3 {
...
func TestBlock_BattlefrontBastionAloneFiresBesideDR(t *testing.T) {
	...
	got := sim.EvalOneTurnForTesting(d, sim.Matchup{IncomingDamage: 10}, gameengine.Spec{}, hand).Value
	if got != 7 {
