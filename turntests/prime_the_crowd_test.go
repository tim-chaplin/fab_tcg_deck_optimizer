...
func TestPrimeTheCrowd_FiresCrowdCheerTrigger(t *testing.T) {
	...
	ge.CreateTrigger(testutils.FakeRedAction(), triggertype.CrowdCheer, func(_ card.GameEngine, _ card.Logger, _ card.EphemeralTrigger, _ *card.CardState, _ triggertype.Type) {
		cheers++
	}, nil)
	ge.ResolveAttackStep(ge.Logger(), &card.CardState{Card: cards.PrimeTheCrowdRed{}})
	if cheers != 1 {
