...
func TestBest_DefenseCappedAtIncoming(t *testing.T) {
	...
	h := []deck.Card{testutils.BlueAttack{}, testutils.BlueAttack{}, testutils.BlueAttack{}, testutils.BlueAttack{}}
	d := deck.New(testutils.Hero{Intel: 4}, nil, nil)
	got := sim.EvalOneTurnForTesting(d, sim.Matchup{IncomingDamage: 2}, nil, h)
	if got.Value != 4 {
...
func TestBest_PlainBlockStillFree(t *testing.T) {
	...
	d := deck.New(testutils.Hero{Intel: 4}, nil, nil)
	got := sim.EvalOneTurnForTesting(d, sim.Matchup{IncomingDamage: 1}, nil, h)
	if got.Value != 1 {
...
func TestBest_RespectsResourceConstraint(t *testing.T) {
	...
	h := []deck.Card{testutils.RedAttack{}, testutils.RedAttack{}, testutils.RedAttack{}, testutils.RedAttack{}}
	d := deck.New(testutils.Hero{Intel: 4}, nil, nil)
	got := sim.EvalOneTurnForTesting(d, sim.Matchup{IncomingDamage: 0}, nil, h)
	if got.Value != 6 {
		t.Fatalf("want value 6, got %d", got.Value)
	}
	var res, cost int
...
func TestBest_AllAttackHandPlusArsenalNoWeapons(t *testing.T) {
	...
	d := deck.New(testutils.Hero{Intel: 4}, nil, nil)
	got := sim.EvalOneTurnForTesting(d, sim.Matchup{IncomingDamage: 0}, gameengine.GameStateBuilder().SetArsenal(cards.WoundingBlowRed{}).Build(), h)
	if got.Value != 4 {
