...
func TestHandState_BlockerEmptiesHandForSpringLoad(t *testing.T) {
	...
	_, extras := sim.EvalOneTurnForTesting(d, sim.Matchup{IncomingDamage: 3}, nil, hand)
	if got := extras.Value; got != 7 {
...
func TestHandState_DemolitionCrewSeesUncommittedPitchInHand(t *testing.T) {
	...
	_, extras := sim.EvalOneTurnForTesting(d, sim.Matchup{IncomingDamage: 0}, gameengine.GameStateBuilder().SetArsenal(cards.FlyingHighRed{}).Build(), hand)
	if got := extras.Value; got != 10 {
