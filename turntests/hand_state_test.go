...
func TestHandState_SpringLoadAlonePitchEmptiesHand(t *testing.T) {
	...
	if got := sim.EvalOneTurnForTesting(d, sim.Matchup{IncomingDamage: 0}, sim.TurnState{}, hand).Value; got != 5 {
...
func TestHandState_BlockerEmptiesHandForSpringLoad(t *testing.T) {
	...
	if got := sim.EvalOneTurnForTesting(d, sim.Matchup{IncomingDamage: 3}, sim.TurnState{}, hand).Value; got != 7 {
...
func TestHandState_UpcomingChainStepBlocksFirstSpringLoadRider(t *testing.T) {
	...
	if got := sim.EvalOneTurnForTesting(d, sim.Matchup{IncomingDamage: 0}, sim.TurnState{}, hand).Value; got != 7 {
...
func TestHandState_MidChainDrawBlocksSpringLoadRider(t *testing.T) {
	...
	if got := sim.EvalOneTurnForTesting(d, sim.Matchup{IncomingDamage: 0}, sim.TurnState{}, hand).Value; got != 6 {
...
func TestHandState_HeldCardBlocksSpringLoadRider(t *testing.T) {
	...
	// review.
	if got := sim.EvalOneTurnForTesting(d, sim.Matchup{IncomingDamage: 0}, sim.TurnState{}, hand).Value; got != 2 {
