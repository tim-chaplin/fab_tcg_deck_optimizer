...
func TestWaterTheSeeds_HighPowerFizzles(t *testing.T) {
	s := &gameengine.GameEngine{GameState: gameengine.GameStateBuilder().SetCardsRemaining([]*card.CardState{{Card: testutils.GenericAttack(0, 2)}}).Build()}
	s.ResolveChainStep(s.Logger(), &card.CardState{Card: cards.WaterTheSeedsRed{}})
...
func TestWaterTheSeeds_SkipsPastNonMatchingAttacks(t *testing.T) {
	...
	s := &gameengine.GameEngine{GameState: gameengine.GameStateBuilder().SetCardsRemaining([]*card.CardState{skipped, target}).Build()}
	s.ResolveChainStep(s.Logger(), &card.CardState{Card: cards.WaterTheSeedsRed{}})
