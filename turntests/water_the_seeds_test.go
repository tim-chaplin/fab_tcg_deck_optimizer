...
func TestWaterTheSeeds_HighPowerFizzles(t *testing.T) {
	s := gameengine.NewFromSpec(gameengine.Spec{CardsRemaining: []*card.CardState{{Card: testutils.GenericAttack(0, 2)}}})
...
func TestWaterTheSeeds_SkipsPastNonMatchingAttacks(t *testing.T) {
	...
	s := gameengine.NewFromSpec(gameengine.Spec{CardsRemaining: []*card.CardState{skipped, target}})
	s.ResolveChainStep(s.Logger(), &card.CardState{Card: cards.WaterTheSeedsRed{}})
