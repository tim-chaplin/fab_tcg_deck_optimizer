...
// masterFromItems returns a *GameState seeded with the supplied items. Tests use this to
// pass token carryover (Gold / Silver / Copper) into EvalOneTurnForTesting.
func masterFromItems(items ...*token.Item) *gameengine.GameState {
...
func TestCopperToken_NotEnoughResourceSkipsSpend(t *testing.T) {
	...
	got := sim.EvalOneTurnForTesting(d, sim.Matchup{IncomingDamage: 0}, masterFromItems(priorItems...), hand)
