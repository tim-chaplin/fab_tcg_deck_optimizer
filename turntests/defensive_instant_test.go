...
func TestDefensiveInstant_BrushOffRedAlone(t *testing.T) {
	...
	if got := sim.EvalOneTurnForTesting(d, sim.Matchup{IncomingDamage: 5}, gameengine.Spec{}, hand).Value; got != 3 {
...
func TestDefensiveInstant_BrushOffYellowAndBlueAlone(t *testing.T) {
	d := deck.New(heroes.Viserai{}, nil, fillerDeck())
	if got := sim.EvalOneTurnForTesting(d, sim.Matchup{IncomingDamage: 5}, gameengine.Spec{}, []deck.Card{cards.BrushOffYellow{}}).Value; got != 2 {
		t.Fatalf("Yellow Value = %d, want 2", got)
	}
	if got := sim.EvalOneTurnForTesting(d, sim.Matchup{IncomingDamage: 5}, gameengine.Spec{}, []deck.Card{cards.BrushOffBlue{}}).Value; got != 1 {
...
func TestDefensiveInstant_PreventionCapsAtIncoming(t *testing.T) {
	...
	if got := sim.EvalOneTurnForTesting(d, sim.Matchup{IncomingDamage: 1}, gameengine.Spec{}, hand).Value; got != 1 {
...
func TestDefensiveInstant_PeaceOfMindWithCost(t *testing.T) {
	...
	if got := sim.EvalOneTurnForTesting(d, sim.Matchup{IncomingDamage: 4}, gameengine.Spec{}, hand).Value; got != 4 {
...
func TestDefensiveInstant_StacksWithDR(t *testing.T) {
	...
	if got := sim.EvalOneTurnForTesting(d, sim.Matchup{IncomingDamage: 5}, gameengine.Spec{}, hand).Value; got != 5 {
...
func TestDefensiveInstant_DefendsFromArsenal(t *testing.T) {
	...
	if got := sim.EvalOneTurnForTesting(d, sim.Matchup{IncomingDamage: 3}, gameengine.Spec{Arsenal: cards.BrushOffRed{}}, hand).Value; got != 3 {
