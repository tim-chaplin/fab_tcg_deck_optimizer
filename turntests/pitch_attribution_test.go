...
func TestPitchAttribution_DeathlyDuetBothRidersFireFromMixedFunding(t *testing.T) {
	...
	if got := sim.EvalOneTurnForTesting(d, sim.Matchup{IncomingDamage: 0}, nil, hand).Value; got != 8 {
...
func TestPitchAttribution_OneNonAttackPitchFundsMultipleAetherSlashes(t *testing.T) {
	...
	if got := sim.EvalOneTurnForTesting(d, sim.Matchup{IncomingDamage: 0}, nil, withNonAttack).Value; got != 15 {
	...
	if got := sim.EvalOneTurnForTesting(d, sim.Matchup{IncomingDamage: 0}, nil, withAttack).Value; got != 13 {
