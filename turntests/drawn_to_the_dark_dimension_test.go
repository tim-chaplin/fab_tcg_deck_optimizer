...
func TestDrawnToTheDarkDimension_CostBounds(t *testing.T) {
	...
	for _, c := range cases {
		...
		if c.Cost(&gameengine.GameEngine{GameState: gameengine.NewState()}) != 2 {
			t.Errorf("%s: Cost(zeroState) = %d, want 2", c.Name(), c.Cost(&gameengine.GameEngine{GameState: gameengine.NewState()}))
		}
		withRune := &gameengine.GameEngine{GameState: gameengine.NewState()}
