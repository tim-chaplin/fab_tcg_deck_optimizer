...
func TestBloodspillInvocation_AttackActionHitCreatesRunechants(t *testing.T) {
	...
	for _, tc := range cases {
		ge := &gameengine.GameEngine{GameState: gameengine.GameStateBuilder().
			...
			Build()}
		ge.FireTriggers(card.FireContext{FiringType: triggertype.Hit, TriggeringCard: testutils.AsCardState(testutils.FakeRedAttack().WithTypes(card.TypeRuneblade))})
...
func TestBloodspillInvocation_WeaponHitDoesNotPop(t *testing.T) {
	ge := &gameengine.GameEngine{GameState: gameengine.GameStateBuilder().
		...
		Build()}
	ge.FireTriggers(card.FireContext{FiringType: triggertype.Hit, TriggeringCard: testutils.AsCardState(testutils.FakeWeaponSwing().WithTypes(card.TypeRuneblade))})
