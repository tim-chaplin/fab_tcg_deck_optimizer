...
// IncomingPhysicalDamage has room.
...
// Tests that prevention caps at IncomingPhysicalDamage when incoming is less than Defense().
func TestBrushOff_CapsAtIncoming(t *testing.T) {
	...
	if summary.Value != 1 {
		t.Errorf("Value = %d, want 1 (capped at IncomingPhysicalDamage)", summary.Value)
	}
}
