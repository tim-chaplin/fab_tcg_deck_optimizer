...
- **Aura presence in graveyard is partly assumed.** Sigil of Silphidae and Runic Fellingsong
  (partly) assume there's always an aura available to banish. Weeping Battleground now scans
  `TurnState.Graveyard` for an actual aura and fizzles when none is present; the graveyard is
  populated with this-turn plain blocks, Defense Reactions, and attack-chain plays.
