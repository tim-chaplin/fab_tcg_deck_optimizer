...
### Unimplemented generic Silver-Age cards

The generic cards below are Silver Age legal per `data_sources/card.csv` (surfaced by
`go run ./cmd/parsecarddb -type generic -names_only`) but have no implementation file in
`internal/card/generic/` yet, so they never enter the random-generation or mutation pools.
Filing one means: add an ID in `internal/card/id.go`, create `internal/card/generic/<name>.go`
with a per-colour struct, register the IDs in `internal/cards/index.go`, and — for anything
non-trivial — model the rider or flag it in the "Undervalued / Overvalued" section below.
Buckets reflect each card's `Types` field in the CSV so the modelling burden is grouped.

Items — Action, Item (triggered / activated non-attack plays):
...
Instants (play during any step; no attack chain):
...
Attack Reactions (play during an attack step):
...
Resources (pitch-and-pop Gold / token feeders — each gates on the Gold economy, see "Token
economies" section):
...
Weapons (belong in `internal/weapon/`, not `internal/card/generic/`):

- Talishar, the Lost Prince — 2H sword; {r}{r} + rust-counter activation, self-destructs at
  3 rust. Today the weapon registry has Nebula Blade, Reaping Blade, and Scepter of Pain;
  Talishar needs a new `internal/weapon/talishar.go` plus a `weapon.All` entry.
