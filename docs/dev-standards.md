...
- **Lifecycle is the handler's job.** When done, the handler calls `s.DestroyAura(addToGraveyard)` to splice itself out of `s.Auras` and (when `addToGraveyard`) land `Self` in the graveyard. Counter-based auras decrement `a.Count` and call `DestroyAura` at zero. The sim never mutates `Count` or graveyards on its own.
