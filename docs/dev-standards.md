...
- **`OnHit` registrations**: attack cards with "if this hits, do X" append a `func(*sim.TurnState)` to `self.OnHit` inside `Play`. Chain runner finalizes each attack post-AR-buff: when `LikelyToHit(self)` is true on post-buff `EffectiveAttack`, every closure in `self.OnHit` runs. The handler may also set `FireBlocked` for the rarer "if this is fully blocked, do Y" path (Wage Gold's wager-loss); the runner dispatches `FireBlocked` instead of `Fire` when `LikelyToHit` is false. Either callback may be nil. Cards adding an on-hit rider to a DIFFERENT card (Mauvrion Skies, Runic Reaping) append to the target's `OnHit`. **Do NOT call `LikelyToHit` directly from `Play`** — chain runner owns the gate so AR buffs propagate. Use a top-level handler (not an inline closure) so registration stays allocation-free — closures assigned to `OnHit.Fire` / `OnHit.FireBlocked` escape to the heap.
- **`NextAttackActionHit` triggers** (Plunder Run, …): cards whose printed text reads "the next time an attack action card you control hits this turn, do X" register via `s.RegisterNextAttackActionHit(...)`. Chain runner drains the queue inside `finalizeActiveAttack` on the first attack action that lands (`IsAttackAction` + `LikelyToHit`); all listeners fire together. Use this — not `OnHit` on a specific `CardState` — when the rider must wait across misses for the FIRST attack action that hits.
- **Self-granting on-hit go-again** (Overload, Razor Reflex mode 1): "if this hits, it gains go again" → flip `self.GrantedGoAgain = true` inside `Play` when `sim.LikelyToHit(self)` returns true. Carry the `sim.ConditionalGoAgain` marker so the lint test in `conditional_go_again_test.go` passes.
...
`Card.Play` uses two orthogonal `TurnState` primitives: `AddValue(n)` mutates `s.Value` (positive credits only — clamps negatives to no-op); `Log` / `LogRider` / `LogPreTrigger` / `LogPostTrigger` (plus `f` variants) append a `LogEntry`. The `skipLog` gate lives inside the Log helpers — cards never check it. Cards that explicitly hand value to the opponent (Wage Gold's wager loss) call `AddOpponentValue(n)` instead — same shape as `AddValue`, subtracts `n` from `s.Value`, and is the only path that can drive the running total negative.
