...
// LegalCards returns every registered card eligible for deck construction. Allocates
// O(legal card count) per call; deck.Random calls it once per sample run.
func (Registry) LegalCards() []Card {
	out := make([]Card, 0, len(cardsByID))
...
func (Registry) LegalWeapons() []Weapon {
