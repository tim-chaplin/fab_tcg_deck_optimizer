...
// is typed as `any` so item carries no v2/card dependency. Callers type-assert the ability
// to their richer card type when invoking.
//
// Constructors are typed by source: NewFromToken wraps the built-in FaB tokens (Gold /
// Silver / Copper, sourced from internal/cards via v2/token). A NewFromCard sibling will
...
// activated-ability card, and initial count. Production callers reach for v2/token's
