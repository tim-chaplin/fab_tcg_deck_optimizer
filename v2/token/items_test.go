...
// deckOf builds a *deck.Deck whose draw order is the supplied cards. Tests use it as a
// one-line stand-in to seed a non-empty deck before a DrawOne.
...
func TestGoldToken_PlaysDecrementsAndDestroys(t *testing.T) {
	ge := gameengine.New()
	ge.SetDeck(deckOf(testutils.RedAttack{}))
