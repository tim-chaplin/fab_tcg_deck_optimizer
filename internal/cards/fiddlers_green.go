...
func fiddlersGreenPlay(s sim.GameEngine, l sim.Logger, self *sim.CardState, heal int) {
	s.AddValue(heal)
	l.AppendPostTriggerf(self.Card.DisplayName(), heal, "Gained %d health (graveyard trigger)", heal)
}

func (FiddlersGreenRed) Play(s sim.GameEngine, l sim.Logger, self *sim.CardState) {
	fiddlersGreenPlay(s, l, self, 3)
}

func (FiddlersGreenYellow) Play(s sim.GameEngine, l sim.Logger, self *sim.CardState) {
	fiddlersGreenPlay(s, l, self, 2)
}

func (FiddlersGreenBlue) Play(s sim.GameEngine, l sim.Logger, self *sim.CardState) {
