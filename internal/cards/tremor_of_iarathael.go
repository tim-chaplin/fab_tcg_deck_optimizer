...
func tremorOfIArathaelPlay(s sim.GameEngine, l sim.Logger, self *sim.CardState) {
	if s.CardBanished() {
		self.BonusAttack += 2
	}
}

func (TremorOfIArathaelRed) Play(s sim.GameEngine, l sim.Logger, self *sim.CardState) {
	tremorOfIArathaelPlay(s, l, self)
}

func (TremorOfIArathaelYellow) Play(s sim.GameEngine, l sim.Logger, self *sim.CardState) {
	tremorOfIArathaelPlay(s, l, self)
}

func (TremorOfIArathaelBlue) Play(s sim.GameEngine, l sim.Logger, self *sim.CardState) {
	tremorOfIArathaelPlay(s, l, self)
}
