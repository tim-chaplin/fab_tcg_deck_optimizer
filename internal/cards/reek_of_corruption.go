...
func reekOfCorruptionApplyRider(s sim.GameEngine, l sim.Logger, self *sim.CardState) {
...
func reekOfCorruptionOnHit(s sim.GameEngine, l sim.Logger, self *sim.CardState, _ *sim.OnHitHandler) {
	s.AddValue(sim.DiscardValue)
	l.AppendPostTrigger(self.Card.DisplayName(), "On-hit discarded a card", sim.DiscardValue)
}

func (ReekOfCorruptionRed) Play(s sim.GameEngine, l sim.Logger, self *sim.CardState) {
	reekOfCorruptionApplyRider(s, l, self)
}

func (ReekOfCorruptionYellow) Play(s sim.GameEngine, l sim.Logger, self *sim.CardState) {
	reekOfCorruptionApplyRider(s, l, self)
}

func (ReekOfCorruptionBlue) Play(s sim.GameEngine, l sim.Logger, self *sim.CardState) {
	reekOfCorruptionApplyRider(s, l, self)
}
