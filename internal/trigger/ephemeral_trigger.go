...
func NewEphemeralTrigger(source card.Card, tt triggertype.Type, fire func(card.GameEngine, card.Logger, card.EphemeralTrigger, *card.CardState, triggertype.Type), typeFilter TypeFilter) *EphemeralTrigger {
...
// triggering card (or nil for turn-boundary events) and the firing type so multi-trigger
// handlers can dispatch on which event invoked them.
func (e *EphemeralTrigger) Fire(engine card.GameEngine, logger card.Logger, triggeringCard *card.CardState, firingType triggertype.Type) {
	e.Invoke(engine, logger, e, triggeringCard, firingType)
}
