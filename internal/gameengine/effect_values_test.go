...
func TestLikelyToHit_FoldsEffectiveAttackAndDominate(t *testing.T) {
	...
	for _, tc := range cases {
		var c card.Card = fakeCard{name: tc.name, attack: tc.printed}
		if tc.printedDom {
			c = dominatingFake{fakeCard{name: tc.name, attack: tc.printed}}
		}
		p := &card.CardState{Card: c, Ephemeral: card.Ephemeral{BonusAttack: tc.bonusAttack, GrantedDominate: tc.grantedDom}}
