...
- **`Aura` / `EphemeralTrigger` / `Item` / `Hero`** (`interfaces.go`) — the engine's local
  ...
  `sim` wires builders in via `init` so the engine never imports the concrete types.
...
walks auras, ephemeral triggers, and items via `fireHooks`, which: takes a length snapshot up
front (so an entry a handler creates lands past it and is not fired this pass — the
