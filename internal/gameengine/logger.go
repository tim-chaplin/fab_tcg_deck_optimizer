...
// path installs this on the replay state so chain steps and trigger riders land in the
// writer as they happen — no accumulation, no buffering. AmendLastChainStepN is a no-op:
// the prior chain-step line has already been written, so AR buffs surface as their own
...
// appendTrigger renders a trigger line indented to sit beneath its chain step. The source
// tag is omitted from the wire form because the indentation already conveys "child of the
// prior chain step"; orphan triggers (no matching parent) would lose attribution under
// this rule, but the chain runner pairs them tightly so orphans don't occur in practice.
