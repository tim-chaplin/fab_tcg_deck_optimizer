...
type attackBufs struct {
	...
	weaponCosts []int
	weaponNames [][]string
	// permMeta parallels perm: precomputed scalar metadata per attacker so playSequence's inner
	// loop skips interface dispatch on Types / Cost / GoAgain / the DiscountPerRunechant
	// assertion. bestSequence swaps perm and permMeta together.
