...
func TestIsLegalOrder_MauvrionCantSaveShrillWhenRuneragerIsAhead(t *testing.T) {
	...
	if _, _, _, legal := playSequence(hero.Viserai{}, nil, nil, order, pcBuf, ptrBuf, cpBuf, state, 1_000_000, 0); legal {
...
func TestBestAttackDamage_PlayedCardGrantsDontLeakAcrossPermutations(t *testing.T) {
	...
	_, _, _ = bestAttackDamage(stubHero{}, attackers, nil, nil, newAttackBufs(0, len(attackers), nil), 1_000_000, 0)
	if sawLeak {
