...
of the attack chain that maximises turn value. Turn value is damage dealt plus damage
...
2. **Attack-chain search** (`sequence.go`) — for one partition leaf, `bestAttackWithWeapons`
...
`playSequenceWithMeta` is the chain runner: it builds a fresh per-permutation `*GameState`,
removes each played card from hand, funds it through the `pitchPool`, spends an action point
unless the step is free, fires `CardOrAbility` triggers, calls `ResolveChainStep`, finalizes
each attack's on-hit effects, and fires `EndOfTurn`. Every leaf and permutation is scored by
`chainScore` — a lexicographic tuple of `(value, cardsPlayed, totalCards, totalCounters)` —
...
- **`TurnSummary`** (`hand_types.go`) — the result of one turn: the winning `BestLine` of
  role assignments, swung weapon names, `Value`, and the post-chain `*GameState`.
...
attack/DR/modal flags) so the chain inner loop avoids interface dispatch. The metadata table
...
`ResolveChainStep` (on `*gameengine.GameEngine`, called by the chain runner and the defense
pass) runs `card.Play`, then credits `g.Value` and appends the canonical
`<Card>: <VERB> (+N)` chain-step log entry. `N` is `EffectiveAttack` for attacks and weapon
...
A card's `Play` body does **not** emit its own chain step — `ResolveChainStep` owns that.
...
Conditional self-buffs go in the `Play` body, *before* the chain step is logged —
`ResolveChainStep` reads the buffed `EffectiveAttack` / `EffectiveDefense` after `Play`
returns:
...
- `evaluate_partition.go` — the partition-leaf → chain-search bridge.
...
- New `turntests/` files must not call `ResolveChainStep` directly — they exercise the
