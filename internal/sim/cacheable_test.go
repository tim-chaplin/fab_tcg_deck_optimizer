...
func TestBest_CacheableEmptyHand(t *testing.T) {
	got := Best(nil, nil, Matchup{IncomingDamage: 0}, nil, Prior{Hero: testutils.Hero{Intel: 4}})
	if !got.Cacheable {
...
func TestBest_UncacheableSkyFireLanterns(t *testing.T) {
	...
	got := Best(nil, h, Matchup{IncomingDamage: 0}, deck, Prior{Hero: testutils.Hero{Intel: 4}})
	if got.Cacheable {
...
func TestBest_UncacheableWeepingBattlegroundDR(t *testing.T) {
	...
	got := Best(nil, h, Matchup{IncomingDamage: 3}, nil, Prior{Hero: testutils.Hero{Intel: 4}})
	if got.Cacheable {
...
func TestBest_ResetBetweenCallsClearsCacheableState(t *testing.T) {
	...
	first := ev.Best(nil, []card.Card{cards.SkyFireLanternsRed{}}, Matchup{IncomingDamage: 0}, deck, Prior{Hero: testutils.Hero{Intel: 4}})
	if first.Cacheable {
	...
	clean := ev.Best(nil, []card.Card{testutils.RedAttack{}}, Matchup{IncomingDamage: 0}, deck, Prior{Hero: testutils.Hero{Intel: 4}})
	if !clean.Cacheable {
