...
func TestBest_CacheableEmptyHand(t *testing.T) {
	got := Best(nil, nil, Matchup{IncomingDamage: 0}, nil, gameengine.GameStateBuilder().SetHero(testutils.Hero{Intel: 4}).Build())
	if !got.Cacheable {
...
func TestBest_UncacheableMoonWishTutor(t *testing.T) {
	h := []card.Card{cards.MoonWishRed{}, testutils.RedAttack{}}
...
func TestBest_UncacheableRavenousRabble(t *testing.T) {
	...
	got := Best(nil, h, Matchup{IncomingDamage: 0}, DeckOf(testutils.GenericAttackPitch(0, 0, 1)), gameengine.GameStateBuilder().SetHero(testutils.Hero{Intel: 4}).Build())
	if got.Cacheable {
...
func TestBest_UncacheableTestOfStrengthClash(t *testing.T) {
	...
	got := Best(nil, h, Matchup{IncomingDamage: 1}, DeckOf(testutils.GenericAttack(0, 7)), gameengine.GameStateBuilder().SetHero(testutils.Hero{Intel: 4}).Build())
	if got.Cacheable {
...
func TestBest_UncacheableWeepingBattlegroundDR(t *testing.T) {
	...
	got := Best(nil, h, Matchup{IncomingDamage: 3}, nil, gameengine.GameStateBuilder().SetHero(testutils.Hero{Intel: 4}).Build())
	if got.Cacheable {
...
func TestBest_AggregationDeckReaderInHandPoisonsResultEvenWhenPitched(t *testing.T) {
	h := []card.Card{cards.SkyFireLanternsBlue{}, testutils.RedAttack{}, testutils.RedAttack{}, testutils.RedAttack{}}
	deck := DeckOf(testutils.RedAttack{})
	got := Best(nil, h, Matchup{IncomingDamage: 0}, deck, gameengine.GameStateBuilder().SetHero(testutils.Hero{Intel: 4}).Build())
	if got.Cacheable {
...
func TestBest_ResetBetweenCallsClearsCacheableState(t *testing.T) {
	...
	first := ev.Best(nil, []card.Card{cards.SkyFireLanternsRed{}}, Matchup{IncomingDamage: 0}, deck, gameengine.GameStateBuilder().SetHero(testutils.Hero{Intel: 4}).Build())
	if first.Cacheable {
	...
	clean := ev.Best(nil, []card.Card{testutils.RedAttack{}}, Matchup{IncomingDamage: 0}, deck, gameengine.GameStateBuilder().SetHero(testutils.Hero{Intel: 4}).Build())
	if !clean.Cacheable {
