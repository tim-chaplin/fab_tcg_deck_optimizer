...
func TestBest_CacheableEmptyHand(t *testing.T) {
	got := Best(testutils.Hero{Intel: 4}, nil, nil, Matchup{IncomingDamage: 0}, nil, nil, nil, nil)
	if !got.Cacheable {
...
func TestBest_UncacheableTestOfStrengthClash(t *testing.T) {
	...
	got := Best(testutils.Hero{Intel: 4}, nil, h, Matchup{IncomingDamage: 1}, deck, nil, nil, nil)
	if got.Cacheable {
...
func TestBest_UncacheableWeepingBattlegroundDR(t *testing.T) {
	...
	got := Best(testutils.Hero{Intel: 4}, nil, h, Matchup{IncomingDamage: 3}, nil, nil, nil, nil)
	if got.Cacheable {
...
func TestBest_ResetBetweenCallsClearsCacheableState(t *testing.T) {
	...
	first := ev.Best(testutils.Hero{Intel: 4}, nil, []Card{cards.SkyFireLanternsRed{}}, Matchup{IncomingDamage: 0}, deck, nil, nil, nil)
	if first.Cacheable {
	...
	clean := ev.Best(testutils.Hero{Intel: 4}, nil, []Card{testutils.RedAttack{}}, Matchup{IncomingDamage: 0}, deck, nil, nil, nil)
	if !clean.Cacheable {
