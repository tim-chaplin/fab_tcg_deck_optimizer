...
func TestBest_CacheableEmptyHand(t *testing.T) {
	got := Best(testutils.Hero{Intel: 4}, nil, nil, Matchup{IncomingDamage: 0}, nil, nil)
	if !got.Cacheable {
...
func TestBest_UncacheableWeepingBattlegroundDR(t *testing.T) {
	...
	got := Best(testutils.Hero{Intel: 4}, nil, h, Matchup{IncomingDamage: 3}, nil, nil)
	if got.Cacheable {
...
func TestBest_ResetBetweenCallsClearsCacheableState(t *testing.T) {
	...
	first := ev.Best(testutils.Hero{Intel: 4}, nil, []Card{cards.SkyFireLanternsRed{}}, Matchup{IncomingDamage: 0}, deck, nil)
	if first.Cacheable {
	...
	clean := ev.Best(testutils.Hero{Intel: 4}, nil, []Card{testutils.RedAttack{}}, Matchup{IncomingDamage: 0}, deck, nil)
	if !clean.Cacheable {
