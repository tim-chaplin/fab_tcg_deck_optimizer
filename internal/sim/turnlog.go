...
func appendDefenseReactionLines(out []string, a deck.CardAssignment, defenders []card.Card, remaining int) ([]string, int) {
	...
	ge.ResolveChainStep(tl, &cs)
	return appendGroupedChainEntries(out, tl.Entries()), ge.RemainingUnblockedDamage()
}
...
// FormatLogEntry renders an orphan trigger LogEntry as "<Text> [(+N)] (from <Source>)" so
// the producer's identity survives when the entry has no matching chain step to group
// under. Chain-step entries with N=0 drop the "(+0)" suffix.
