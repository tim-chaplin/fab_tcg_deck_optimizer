...
func TestFormatBestTurn_LogAttributesEachTriggerSeparately(t *testing.T) {
	...
	got := Best(heroes.Viserai{}, nil, h, Matchup{}, nil, nil, prior, nil)
	out := FormatBestTurn(got, nil, nil)
...
func TestFormatBestTurn_MoonWishTutorAndPlayLogsAsPostTrigger(t *testing.T) {
	...
	got := Best(testutils.Hero{Intel: 4}, nil, h, Matchup{IncomingDamage: 0}, deck, nil, nil, nil)
	out := FormatBestTurn(got, nil, nil)
	wants := []string{
...
func TestFormatBestTurn_ArsenalInPlayedAsDR(t *testing.T) {
	...
	got := Best(testutils.Hero{Intel: 4}, nil, h, Matchup{IncomingDamage: 4}, nil, cards.ToughenUpBlue{}, nil, nil)
	out := FormatBestTurn(got, nil, nil)
	if !strings.Contains(out, "  Opponent's turn:") {
...
func TestFormatBestTurn_DefenseReactionLinesAndRiders(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			got := Best(testutils.Hero{Intel: 4}, nil, tc.hand, Matchup{IncomingDamage: tc.incoming}, nil, nil, nil, nil)
			out := FormatBestTurn(got, nil, nil)
...
func TestFormatBestTurn_ArsenalInPlayedOnChain(t *testing.T) {
	...
	got := Best(testutils.Hero{Intel: 4}, nil, h, Matchup{IncomingDamage: 0}, nil, testutils.RedAttack{}, nil, nil)
	out := FormatBestTurn(got, nil, nil)
	if !strings.Contains(out, "  My turn:") {
		t.Errorf("want 'My turn:' section header, got:\n%s", out)
	}
...
func TestFormatBestTurn_WeaponSwingInChain(t *testing.T) {
	...
	got := Best(testutils.Hero{Intel: 4}, weapons, h, Matchup{IncomingDamage: 0}, nil, nil, nil, nil)
	out := FormatBestTurn(got, nil, nil)
...
func TestFormatBestTurn_EndOfTurnArsenalStayed(t *testing.T) {
	...
	h := []Card{cards.ToughenUpBlue{}}
	got := Best(testutils.Hero{Intel: 4}, nil, h, Matchup{IncomingDamage: 0}, nil, cards.ToughenUpBlue{}, nil, nil)
	out := FormatBestTurn(got, nil, nil)
...
func TestFormatBestTurn_EndOfTurnAurasWithRunechants(t *testing.T) {
	summary := TurnSummary{
		State: CarryState{
			Auras: []Aura{
				{Self: CardOrTokenType{Card: cards.MaleficIncantationRed{}}},
				NewRunechantAura(2),
			},
		},
	}
