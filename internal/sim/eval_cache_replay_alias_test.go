...
func TestEvalCacheReplay_PromotesWinnerSlices(t *testing.T) {
	...
	cachedStats := NewEvaluator().Evaluate(baseline.Copy(), 20, Matchup{IncomingDamage: 7}, rand.New(rand.NewSource(99)))
	uncachedStats := NewEvaluatorWithoutCache().Evaluate(baseline.Copy(), 20, Matchup{IncomingDamage: 7}, rand.New(rand.NewSource(99)))
