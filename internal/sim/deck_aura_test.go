...
func TestProcessAurasAtStartOfTurn_GraveyardsExhaustedAura(t *testing.T) {
	...
	watcher := aura.NewFromCard(
		...
		triggertype.StartOfTurn,
		func(ge card.GameEngine, _ card.Logger, _ card.Aura, _ *card.CardState, _ triggertype.Type) {
	...
	first := aura.NewFromCard(
		src,
		triggertype.StartOfTurn,
		func(_ card.GameEngine, _ card.Logger, a card.Aura, _ *card.CardState, _ triggertype.Type) {
			a.Destroy(true)
		},
		1,
		false,
