...
type TurnState struct {
	...
	// when an attack or weapon swing actually lands (LikelyToHit) — modelled as the
	// opponent's first incoming physical damage stripping the mark. Arcane damage
	// (Runechants, arcane riders) and non-landing swings don't clear the mark. Read by
	// "if the defending hero is marked" riders. Our own hero is never modelled as marked,
	// so "if you are marked, can't play" gates on opposing cards drop out. Carries across
	// turns via CarryState so next turn's chain seed sees a Mark applied late in the
	// previous turn.
	OpponentMarked bool
