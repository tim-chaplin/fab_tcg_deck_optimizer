...
func TestDrawOne_AppendsTopAndAdvancesDeck(t *testing.T) {
	...
	if got := s.Deck().Size(); got != 2 {
		t.Fatalf("Deck size = %d, want 2", got)
	}
	if top, _ := s.PeekDeck(); top != b {
		t.Errorf("Deck top = %v, want b (top advanced past a)", top)
	}
...
func TestAddAura_FlipsAuraCreatedAndAppends(t *testing.T) {
	...
	s.CreateStartOfTurnAura(cs, nil, 2)
	s.CreateStartOfTurnAura(cs, nil, 1)
	if !s.AuraCreated() {
...
func TestHasPlayedType_ScansCardsPlayed(t *testing.T) {
	...
	s.SetCardsPlayed([]card.Card{attack, aura})
	if !s.HasPlayedType(card.TypeAura) {
...
// TestAuraCreated_FlagOnly: AuraCreated reflects the sticky flag set as a side effect of
// Create*Aura calls and by ResolveChainStep when an Aura-typed card resolves.
func TestAuraCreated_FlagOnly(t *testing.T) {
	var empty TurnState
	if empty.AuraCreated() {
		t.Error("no aura, no flag → should be false")
	}

	flagged := NewTurnStateFromSpec(TurnStateSpec{AuraCreated: true})
	if !flagged.AuraCreated() {
		t.Error("AuraCreated=true → should be true")
	}
}

// TestAddValue_SumsBothDirections: AddValue sums positive and negative credits into Value.
// Negative grants (Test of Strength's clash-loss conceding value to opponent) move the
// total down; zero is a no-op.
func TestAddValue_SumsBothDirections(t *testing.T) {
	cases := []struct {
		name string
		bump int
		want int
	...
	// Mixed sequence: every credit moves the total.
	var s TurnState
	s.AddValue(2)
	s.AddValue(-10)
	s.AddValue(0)
	s.AddValue(5)
	if s.Value() != -3 {
		t.Errorf("after mixed sequence Value = %d, want -3 (2-10+0+5)", s.Value())
	}
}
...
func TestCreateRunechants_CountAndFlag(t *testing.T) {
	...
	if s.RunechantCount() != 0 {
		t.Errorf("Runechants = %d, want 0", s.RunechantCount())
	}
	if s.Value() != 0 {
	...
	if s.RunechantCount() != 3 {
		t.Errorf("Runechants = %d, want 3", s.RunechantCount())
	}
	if s.Value() != 3 {
	...
	if s.RunechantCount() != 5 {
		t.Errorf("Runechants after second call = %d, want 5", s.RunechantCount())
	}
	if s.Value() != 5 {
...
func TestCreatePonder_BumpsCountAndFlag(t *testing.T) {
	...
	if s.PonderCount() != 0 {
		t.Errorf("Ponders = %d, want 0", s.PonderCount())
	}
	...
	if s.PonderCount() != 2 {
		t.Errorf("Ponders = %d, want 2", s.PonderCount())
	}
	if s.Value() != 0 {
	...
	s.CreatePonder(3)
	if s.PonderCount() != 5 {
		t.Errorf("Ponders after second call = %d, want 5", s.PonderCount())
	}
}
...
func TestClash_WinTieLose(t *testing.T) {
	...
	for _, tc := range cases {
		var s *TurnState
		if tc.deckLen > 0 {
			s = NewTurnStateFromCards([]card.Card{NewFakeCard("top").WithAttack(tc.topAtk)}, nil)
		} else {
			s = &TurnState{}
		}
		var winN, loseN int
...
func TestIsCacheable_DeckReadFlips(t *testing.T) {
	s := NewTurnStateFromCards([]card.Card{NewFakeCard("x")}, nil)
	_ = s.Deck()
	if s.IsCacheable() {
...
func TestIsCacheable_GraveyardReadFlips(t *testing.T) {
	s := NewTurnState(nil, []card.Card{NewFakeCard("g")})
	_ = s.Graveyard()
	if s.IsCacheable() {
...
func TestIsCacheable_PopDeckTopFlips(t *testing.T) {
	a := NewFakeCard("a")
	s := NewTurnStateFromCards([]card.Card{a, NewFakeCard("b")}, nil)
	got, ok := s.PopDeckTop()
	if !ok || got != a {
...
func TestIsCacheable_PrependToDeckFlips(t *testing.T) {
	...
	if d := s.Deck(); d.Size() != 2 || d.PeekTop() != added {
		t.Errorf("Deck size=%d, top=%v want size=2 top=y after PrependToDeck", d.Size(), d.PeekTop())
	}
}
...
func TestIsCacheable_TutorFromDeckFlips(t *testing.T) {
	...
	s := NewTurnStateFromCards([]card.Card{NewFakeCard("a"), target, NewFakeCard("b")}, nil)
	got, ok := s.TutorFromDeck(func(c card.Card) int {
		if c == target {
			return 1
		}
		return 0
	...
	if d := s.Deck(); d.Size() != 2 {
		t.Errorf("Deck size after tutor = %d, want 2 cards remaining", d.Size())
	}
}
...
func TestIsCacheable_TutorFromDeckNoMatchFlips(t *testing.T) {
	s := NewTurnStateFromCards([]card.Card{NewFakeCard("a"), NewFakeCard("b")}, nil)
	if got, ok := s.TutorFromDeck(func(card.Card) int { return 0 }); got != nil || ok {
...
func TestIsCacheable_BanishFromGraveyardFlips(t *testing.T) {
	target := NewFakeCard("target")
	s := NewTurnState(nil, []card.Card{NewFakeCard("a"), target})
	got, ok := s.BanishFromGraveyard(func(c card.Card) bool { return c == target })
	if !ok || got != target {
...
func TestIsCacheable_BanishFromGraveyardNoMatchFlips(t *testing.T) {
	s := NewTurnState(nil, []card.Card{NewFakeCard("a")})
	if got, ok := s.BanishFromGraveyard(func(card.Card) bool { return false }); got != nil || ok {
...
func TestIsCacheable_ClashFlipsThroughDeck(t *testing.T) {
	s := NewTurnStateFromCards([]card.Card{NewFakeCard("top").WithAttack(7)}, nil)
	var won bool
...
func TestIsCacheable_NewTurnStateSeedingDoesNotFlip(t *testing.T) {
	s := NewTurnStateFromCards(
		[]card.Card{NewFakeCard("x")},
		[]card.Card{NewFakeCard("y")},
	)
	if !s.IsCacheable() {
