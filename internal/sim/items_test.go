...
func TestEvalCache_PriorItemsKeyedDistinctly(t *testing.T) {
	...
	_ = ev.Best(testutils.Hero{Intel: 4}, nil, hand, mp, nil, nil, nil, []Item{NewGoldItem(1)})
	_ = ev.Best(testutils.Hero{Intel: 4}, nil, hand, mp, nil, nil, nil, []Item{NewGoldItem(2)})
	stats := ev.CacheStats()
