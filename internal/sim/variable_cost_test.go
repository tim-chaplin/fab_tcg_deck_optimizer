...
func TestPlaySequence_DiscountRejectsInsufficientBudget(t *testing.T) {
	order := []Card{cards.AmplifyTheArknightRed{}} // printed cost 3, MinCost 0
...
func TestPlaySequence_DiscountAffordableWithBudget(t *testing.T) {
	order := []Card{cards.AmplifyTheArknightRed{}}
...
func TestPlaySequence_LeftoverFromNonAttackAction(t *testing.T) {
	order := []Card{cards.ReadTheRunesRed{}} // creates 3 runechants, not an attack
	ctx := NewSequenceContextForTest(heroes.Viserai{}, nil, nil, 0, 0, len(order))
	dmg, leftover, _, legal := ctx.PlaySequence(order)
	if !legal {
		t.Fatalf("expected legal sequence")
...
func TestBest_MauvrionReadNoCarryover(t *testing.T) {
	...
	got := Best(heroes.Viserai{}, nil, h, Matchup{IncomingDamage: 0}, nil, nil, nil, nil)
	if got.Value != 4 {
...
func TestBest_MauvrionReadWithCarryover(t *testing.T) {
	h := []Card{cards.MauvrionSkiesRed{}, cards.ReadTheRunesRed{}}
	got := Best(heroes.Viserai{}, nil, h, Matchup{IncomingDamage: 0}, nil, nil, withRunechants(1), nil)
...
func TestBest_AetherSlashAloneConsumesCarryover(t *testing.T) {
	...
	got := Best(heroes.Viserai{}, weapons, h, Matchup{IncomingDamage: 0}, nil, nil, withRunechants(1), nil)
	if got.Value != 3 {
		t.Errorf("Value = %d, want 3 (Reaping Blade attack; carryover consumed without credit)", got.Value)
	}
	if got.State.Runechants() != 0 {
		t.Errorf("leftover Runechants = %d, want 0 (weapon swing consumed the carryover)", got.State.Runechants())
	}
}
...
func TestBest_BlessingOfOccultTokensDoNotAffectSameTurnChain(t *testing.T) {
	...
	got := Best(testutils.Hero{Intel: 4}, nil, h, Matchup{IncomingDamage: 0}, nil, nil, nil, nil)
	if got.Value != 0 {
...
func TestBest_ReduceToRunechantAffordableWithCarryover(t *testing.T) {
	...
	got := Best(heroes.Viserai{}, nil, h, Matchup{IncomingDamage: 4}, nil, nil, withRunechants(1), nil)
	if got.Value != 5 {
...
func TestBest_ReduceToRunechantUnaffordableWithoutCarryover(t *testing.T) {
	h := []Card{cards.ReduceToRunechantRed{}}
	got := Best(heroes.Viserai{}, nil, h, Matchup{IncomingDamage: 4}, nil, nil, nil, nil)
	if got.Value != 0 {
...
func TestBest_DiscountAttackerPaysByPartialCarryoverAndTightPitch(t *testing.T) {
	...
	got := Best(testutils.Hero{Intel: 4}, nil, h, Matchup{IncomingDamage: 0}, nil, nil, withRunechants(2), nil)
	if got.Value != 6 {
		t.Errorf("Value = %d, want 6", got.Value)
	}
}
...
func TestBest_DiscountDefenderPaysByPitchWithoutCarryover(t *testing.T) {
	...
	got := Best(testutils.Hero{Intel: 4}, nil, h, Matchup{IncomingDamage: 4}, nil, nil, nil, nil)
	if got.Value != 5 {
...
func TestBest_CarryoverFeedsDiscount(t *testing.T) {
	...
	got = Best(testutils.Hero{Intel: 4}, nil, h, Matchup{IncomingDamage: 0}, nil, nil, withRunechants(3), nil)
	if got.Value != 6 {
		t.Errorf("carryover=3: Value = %d, want 6 (Attack only; carryover tokens don't re-credit)", got.Value)
	}
	if got.State.Runechants() != 0 {
		t.Errorf("carryover=3: leftover Runechants = %d, want 0 (attack consumes tokens)", got.State.Runechants())
	}
}
