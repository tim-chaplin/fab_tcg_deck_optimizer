...
func TestWeepingBattleground_AuraInGraveyard(t *testing.T) {
	...
	if got := (WeepingBattlegroundRed{}).Play(&s, nil); got != 1 {
...
func TestWeepingBattleground_NoAuraInGraveyard(t *testing.T) {
	...
	if got := (WeepingBattlegroundRed{}).Play(&s, nil); got != 0 {
...
func TestWeepingBattleground_BanishesAura(t *testing.T) {
	...
	(WeepingBattlegroundRed{}).Play(&s, nil)
...
func TestWeepingBattleground_SecondCopyAlsoFires(t *testing.T) {
	...
	s := card.TurnState{Graveyard: []card.Card{aura1, aura2}}
	if got := (WeepingBattlegroundRed{}).Play(&s, nil); got != 1 {
		t.Fatalf("first Play() = %d, want 1", got)
	}
	if got := (WeepingBattlegroundBlue{}).Play(&s, nil); got != 1 {
		t.Fatalf("second Play() = %d, want 1", got)
	}
...
// the first banishes it, the second returns 0.
func TestWeepingBattleground_SecondCopyFizzlesWhenOutOfAuras(t *testing.T) {
	...
	if got := (WeepingBattlegroundBlue{}).Play(&s, nil); got != 0 {
		t.Fatalf("second Play() = %d, want 0 (no aura left)", got)
	}
}

// TestWeepingBattleground_PitchAndDefense: sanity check on the static profile.
func TestWeepingBattleground_PitchAndDefense(t *testing.T) {
	cases := []struct {
		name  string
		card  card.Card
		pitch int
	}{
		{"Red", WeepingBattlegroundRed{}, 1},
		{"Yellow", WeepingBattlegroundYellow{}, 2},
		{"Blue", WeepingBattlegroundBlue{}, 3},
	}
	for _, tc := range cases {
		if got := tc.card.Pitch(); got != tc.pitch {
			t.Errorf("%s: Pitch() = %d, want %d", tc.name, got, tc.pitch)
		}
		if got := tc.card.Defense(); got != 3 {
			t.Errorf("%s: Defense() = %d, want 3", tc.name, got)
		}
		if got := tc.card.Cost(nil); got != 0 {
			t.Errorf("%s: Cost() = %d, want 0", tc.name, got)
		}
		if !tc.card.Types().IsDefenseReaction() {
			t.Errorf("%s: should be a Defense Reaction", tc.name)
		}
	}
}
