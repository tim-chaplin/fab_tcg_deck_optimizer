...
//
// Simplification: create all N Runechants on play rather than next turn's start. We don't model
// cross-turn aura persistence.
//
...
func (BlessingOfOccultRed) Play(s *card.TurnState) int  { return s.CreateRunechants(3) }
...
func (BlessingOfOccultYellow) Play(s *card.TurnState) int { return s.CreateRunechants(2) }
...
func (BlessingOfOccultBlue) Play(s *card.TurnState) int { return s.CreateRunechants(1) }
