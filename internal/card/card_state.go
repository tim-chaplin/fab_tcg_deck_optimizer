...
// entry. p is the Attack Reaction card granting the buff. Positive buffs raise the
// AttackBuffedByReaction trigger with the buffed target as triggering card and n exposed
// via AttackReactionBuffDelta — subscribers (Featherfoot's "exactly +1{p}" gate) filter
// in their own handler so card-specific predicates don't leak into this general helper.
func (p *CardState) GrantAttackReactionBuff(g GameEngine, l Logger, n int) {
	...
	if n > 0 {
		g.RaiseAttackBuffedByReaction(target, n)
	}
}
