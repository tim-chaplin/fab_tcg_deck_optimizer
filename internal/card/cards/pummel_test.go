...
func TestPummel_Mode0AcceptsClubAndHammer(t *testing.T) {
	...
	if !(PummelRed{}).ARTargetAllowed(nil, testutils.FakeWeaponSwing().WithTypes(card.TypeHammer, card.TypeOneHand), 0) {
...
func TestPummel_Mode0RejectsOtherTargets(t *testing.T) {
	if (PummelRed{}).ARTargetAllowed(nil, testutils.FakeRedAttack().WithCost(2), 0) {
...
func TestPummel_Mode1AcceptsCostTwoAttackAction(t *testing.T) {
	if !(PummelRed{}).ARTargetAllowed(nil, testutils.FakeRedAttack().WithCost(2), 1) {
...
func TestPummel_Mode1RejectsCostOneAndNonAttacks(t *testing.T) {
	if (PummelRed{}).ARTargetAllowed(nil, testutils.FakeRedAttack().WithCost(1), 1) {
		t.Error("mode 1 should reject a cost-1 attack action")
	}
	if (PummelRed{}).ARTargetAllowed(nil, testutils.FakeRedAction(), 1) {
