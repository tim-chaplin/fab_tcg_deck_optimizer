...
func lookTuffPlay(ge card.GameEngine, l card.Logger, self *card.CardState) {
	if self.Mode == 0 {
		self.BonusAttack -= 1
	}
}
...
func (LookTuffRed) Play(ge card.GameEngine, l card.Logger, self *card.CardState) {
	lookTuffPlay(ge, l, self)
}
