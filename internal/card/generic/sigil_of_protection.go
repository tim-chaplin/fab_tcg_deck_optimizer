...
// destroyNextTurn is the zero-credit PlayNextTurn result every Sigil of Protection variant uses:
// the Ward rider isn't modelled, so the only job of the callback is to flag the aura as
// destroyed so it lands in the graveyard next turn.
func destroyNextTurn(s *card.TurnState) card.DelayedPlayResult {
	s.DestroyThis()
...
func (SigilOfProtectionRed) PlayNextTurn(s *card.TurnState) card.DelayedPlayResult {
	return destroyNextTurn(s)
}
...
func (SigilOfProtectionYellow) PlayNextTurn(s *card.TurnState) card.DelayedPlayResult {
	return destroyNextTurn(s)
}
...
func (SigilOfProtectionBlue) PlayNextTurn(s *card.TurnState) card.DelayedPlayResult {
	return destroyNextTurn(s)
}
